...
TEST_CASE("can create, save, and load a bitmap", "[bitmap][save][load][get_pixel]")
{
    // Create a small 5x5 bitmap
    string testBitmapName = "test_bitmap";
    bitmap test_bmp = create_bitmap(testBitmapName, 5, 5);
    ...
    SECTION("can create valid bitmap")
    {
        REQUIRE(has_bitmap(testBitmapName));
        REQUIRE(bitmap_width(test_bmp) != 0);
    }
    
    SECTION("bitmap displays pixels correctly")
    {
    ...
    SECTION("can free a bitmap")
    {
        free_bitmap(test_bmp);
        REQUIRE(has_bitmap(testBitmapName) == false);
    }

    SECTION("can save and load a bitmap")
    {
        // Save the bitmap to disk
        string filename = "test_bitmap_pattern_5x5";
        save_bitmap(test_bmp, filename);

        // Get the path to the saved bitmap (should be desktop normally)
        string path = path_from({path_to_user_home(), "Desktop"});

        // Fall back to home if Desktop doesn't exist (mirrors save_bitmap functionality)
        ...
        bitmap loaded_bmp = load_bitmap("loaded_test", filepath);

        // Clean up the unneeded file
        int result = std::remove(filepath.c_str());
    
        // Ensure the bitmap was loaded
        REQUIRE(has_bitmap("loaded_test"));
        REQUIRE(bitmap_width(loaded_bmp) != 0);
